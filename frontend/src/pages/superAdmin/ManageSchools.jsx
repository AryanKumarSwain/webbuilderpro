                        ...
                        <div style={{ overflowX: 'auto' }}>
                            ...
                            return (
                                <div key={school.id} className="school-row"
                                    ...
                                    {/* Actions — a pending school gets here only by not having verified its
                                        signup OTP yet (see signup.service.js); Activate is a manual escape
                                        hatch for a Super Admin to unstick one (e.g. the verification email
                                        never arrived), not an approval step. */}
                                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                                        {school.status === 'active' && (
                                            <a className="action-btn" href={school.custom_domain ? `https://${school.custom_domain}` : `${window.location.origin}/school/${school.slug}`}
                                                target="_blank" rel="noopener noreferrer"
                                                style={{ padding: '5px 10px', background: '#eef2ff', color: '#4338ca', border: '1px solid #c7d2fe', borderRadius: '6px', fontSize: '11px', cursor: 'pointer', fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                                                <svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                                                View
                                            </a>
                                        )}
                                        {school.status !== 'active' && (
